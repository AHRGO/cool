...
In the first week of the course, we had a brief introduction to what is a compiler and of what ("steps") one is made.

Also we've seen some programming in the cool language, the language we will be implementing the compiler on.
...
The students of this course recieve a pre-configured VM with all the setup already configured, but, since this is an online class, it has been made avaliable a tutorial on linux installation, which I decided to try.

In the end I've downloaded the VM, for the setup configuration seemed a bit hard/confused. But the VM is an old linux distro I've never heard about (`todo: write its name here in the future`) and the browser pre-installed doesn't work because it's never been updated (so I can't download new browsers or a newer IDE like VSCode).

Sooooo, I tried again the manual config on my personal machine haha.
...
## The Codespaces problem
I use GitHub codespaces. A lot.

In fact, right now I'm using it to write these notes. 

But, somehow, when I was setting up the environment for the class, I've run into an error when "compiling" the c++ make file from the `/assignments/class` folder. Didn't undestand why, since in my machine it run smoothly.

`todo: describe the bug detailed in the future`

Well, one day I will check it out again and try to solve, then I'll write here what I've discovered and if I had solve the bug. For now, I can live with the fact that I will write the code on codespaces, then pull the updates on my machine and run there.